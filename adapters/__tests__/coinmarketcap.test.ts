...
describe('multi price fetch', () => {
  ...
  it('should throw error if there are missing symbols', async () => {
    ...
    }).rejects.toThrowError('coinmarketcap.missingTickerSymbol')
  })

  it('should throw error if the returned ticker does not match', async () => {
    nock('https://pro-api.coinmarketcap.com')
      .get('/v1/cryptocurrency/quotes/latest?symbol=ETH&CMC_PRO_API_KEY=API_TOKEN')
      .reply(200, function (_) {
        return `{
          "status":{
             "timestamp":"2021-07-28T08:31:48.282Z",
