...
import dex, { getBitcoinPrice } from '../src/dex-cmc'
...
describe('multi price fetch', () => {
  ...
  it('should fetch price from defichain dex using config', async () => {
    ...
    expect(prices).toStrictEqual([
      {
        token: 'DFI',
        amount: new BigNumber('1.8845978905136304014206486861675'),
        currency: 'USD',
...
describe('fetch btc price', () => {
  it('should get cmc btc price', async () => {
    nock('https://pro-api.coinmarketcap.com')
      .get('/v1/cryptocurrency/quotes/latest?symbol=BTC&CMC_PRO_API_KEY=apiToken')
      .reply(200, function (_) {
        return `{
    ...
    const price = await getBitcoinPrice('apiToken')
  ...
  it('should throw error if btc price not returned', async () => {
    ...
    }).rejects.toThrowError('dex-cmc.invalidCMCResponse')
  })
})
